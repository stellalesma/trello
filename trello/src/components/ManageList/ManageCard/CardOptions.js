...
function CardOptions ({ listName, card, onClose }) {
	...
	return (
		<div className="top-0 left-0 w-full h-full z-30 fixed bg-stone-900/50" onClick={handleClickOutside}>
			<div className="top-1/2 left-1/2 w-112.5 h-[90vh] p-5 custom:w-185 transition-spacing -translate-x-2/4 -translate-y-2/4 fixed overflow-y-auto rounded-xl box-border bg-neutral-100" onClick={handleModalClick}>
				<div className="flex items-center justify-between">
					<div className="flex items-center text-xl">
						<IoCardOutline className="mr-2.5 text-3xl" />
						<p title={card.title} className="w-75 custom:w-152.5 font-bold cursor-text truncate">{card.title}</p>
					</div>
					<IoCloseOutline className="text-4xl p-2 rounded-full cursor-pointer hover:bg-stone-200" onClick={onClose} />
				</div>
				<p className="w-64 custom:w-112.5 ml-10 mb-10 text-sm truncate">in list <span title={listName} className="underline cursor-pointer">{listName}</span></p>
